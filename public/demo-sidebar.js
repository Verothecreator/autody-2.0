...
const demoNavIcons = {
  ...
  Rankings: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20V13h4v7M10 20V8h4v12M16 20V4h4v16M3 20h18"/></svg>`,
...
function ensureDemoSidebarTools() {
  ...
  const marketsLink = sidebar.querySelector('.app-nav a[href="account-markets.html"], .app-nav a[href="account-markets"]');
  if (marketsLink && !sidebar.querySelector('[data-account-rankings-link]')) {
    const rankingsLink = document.createElement("a");
    rankingsLink.href = "/account-rankings";
    rankingsLink.textContent = "Rankings";
    rankingsLink.dataset.accountRankingsLink = "true";
    if (/\/account-rankings(?:\.html)?$/.test(location.pathname)) {
      rankingsLink.classList.add("active");
      rankingsLink.setAttribute("aria-current", "page");
      marketsLink.classList.remove("active");
    }
    marketsLink.insertAdjacentElement("afterend", rankingsLink);
  }
  ...
  const isLivePage = page === "account" || page.startsWith("account-");
  const settingsHref = isLivePage ? "account-settings" : "demo-settings";
  const profileHref = isLivePage ? "account-profile" : "demo-profile";
  const profileActive = page === "account-profile" || page === "demo-profile";
